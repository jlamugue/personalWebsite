...
const imagesSwiper = [
	'/src/assets/images/swiper/swiper1.jpg',
...
document.querySelector('#app').append(Card(cards));

function renderCarousel() {
	if (!isMobile()) {
		document.querySelector('#app').append(
			Carousel(carouselImages, {
				visibleCount: 2, // 👈 cambia a 2 o 4 si querés
				gap: 16,
				autoSlide: true,
				interval: 2000,
			})
...
document.querySelector('#app').append(Tab(tabs));

const timelineContainer = document.createElement('div');
timelineContainer.id = 'timeline';
document.querySelector('#app').append(timelineContainer);
let currentTimeline = HorizontalTimeline(steps);
// 2️⃣ Definir la función de renderizado (ya debes tenerla declarada antes de usarla)

// 3️⃣ Agregar el event listener para que se vuelva a renderizar al cambiar tamaño
